...
class DH_HDR_CYC_SSPV(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'G_VGG_recA', 'G_VGG_recB']
        ...
        if self.isTrain:
            ...
            self.netD_B = dh_hdr_networks.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
            self.netD_C = dh_hdr_networks.define_D(opt.input_nc*2, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
    ...
    def forward(self):
        ...
        self.fake_A = self.netG_D_A(self.real_B, self.content_B)
        if self.dh_real:
            self.deh_C = self.netG_D_A(self.real_C, self.content_C)
            self.content_deh_C = self.netG_E_A(self.deh_C)

        # identity #
        self.idt_A = self.netG_D_A(self.real_A, self.content_A)
        ...
        # translation #
        self.rec_A = self.netG_D_A(self.real_A, self.content_fake_B)
    ...
    def backwork_D_C(self):
        ...
        deh_C = self.deh_C_pool.query(self.deh_C)
        dehaze_real_A_real_C_cat = torch.cat((self.real_A, self.real_C), dim=1)
        dehaze_real_A_deh_C_cat = torch.cat((self.real_A, deh_C), dim=1)
        self.loss_D_C = self.backward_D_basic(self.netD_C, dehaze_real_A_real_C_cat, dehaze_real_A_deh_C_cat)
    ...
    def backward_G(self):
        ...
        # Vgg loss
        self.loss_G_VGG_recA = self.criterionVGG(self.rec_A, self.real_A) * self.opt.lambda_vgg
        self.loss_G_VGG_recB = self.criterionVGG(self.rec_B, self.real_B) * self.opt.lambda_vgg
        self.loss_G += self.loss_G_VGG_recA + self.loss_G_VGG_recB
        ...
        if self.dh_real:
            # GAN loss D_B(G_B(C))
            self.loss_G_C = self.criterionGAN(self.netD_C(torch.cat((self.real_A, self.deh_C), dim=1)), True)
