...
class DH_HDR_CYC_USPV(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'idt_A', 'cycle_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'content_A', 'content_B','style_B']
        ...
        if self.isTrain:
            ...
            if self.opt.which_model_netD == 'fd' or self.opt.which_model_netD == 'nl':
                self.netD_A = dh_hdr_networks.define_D(opt.input_nc, opt.ndf,
    ...
    def backward_D_CB(self):
        fake_A = self.fake_A_pool.query(self.fake_A)
        dehaze_real_B_real_A_cat = torch.cat((self.flip(self.real_B), self.real_A), dim=1)
        dehaze_real_B_fake_A_cat = torch.cat((self.flip(self.real_B), fake_A), dim=1)
        self.loss_D_B = self.backward_D_basic(self.netD_B, dehaze_real_B_real_A_cat, dehaze_real_B_fake_A_cat)

    def backward_D_CA(self):
        fake_B = self.fake_B_pool.query(self.fake_B)
        dehaze_real_A_real_B_cat = torch.cat((self.flip(self.real_A), self.real_B), dim=1)
        dehaze_real_A_fake_B_cat = torch.cat((self.flip(self.real_A), fake_B), dim=1)
    ...
    def backward_G(self):
        ...
        if self.opt.which_model_netD == 'fd':
            # GAN loss D_A(G_A(A))
            self.loss_G_A = self.criterionGAN(self.netD_A(self.flip(self.real_D), self.fake_B), True) * 0.218
            #* 2#* 0.382
            # GAN loss D_B(G_B(B))
            self.loss_G_B = self.criterionGAN(self.netD_B(self.flip(self.real_C), self.fake_A), True) * 0.218
            # GAN loss D_A(G_A(A))
            #self.loss_G_PA = self.criterionGAN(self.netD_B(self.real_D, self.fake_PA), True)
        elif self.opt.which_model_netD == 'cd':
            # GAN loss D_A(G_A(A))
            self.loss_G_A = self.criterionGAN(self.netD_A(torch.cat((self.flip(self.real_A), self.fake_B), dim=1)), True) * 0.2018
            #* 0.382
            # GAN loss D_B(G_B(B))
            self.loss_G_B = self.criterionGAN(self.netD_B(torch.cat((self.flip(self.real_B), self.fake_A), dim=1)), True) * 0.2018 
        elif self.opt.which_model_netD == 'nl':
            # GAN loss D_A(G_A(A))
            self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_B), True) * 0.2018
            #* 0.382
            # GAN loss D_B(G_B(B))
            self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True) * 0.2018 
            # GAN loss D_A(G_A(A))
        ...
        # Content Loss
        if self.hl > 0:
            self.loss_content_A = self.criterionIdt(self.content_A[3], self.content_fake_B[3]) * lambda_C
            self.loss_content_B = self.criterionIdt(self.content_B[3], self.content_fake_A[3]) * lambda_C


        if self.opt.which_model_netG == 'unet_fuse_att_msh':
            if self.hl > 0:
                self.loss_style_B = self.criterionIdt(self.style_fake_B[3], self.style_B[3]) * lambda_C * 0.918
            if self.hl > 1:
                self.loss_style_B = self.criterionIdt(self.style_fake_B[2], self.style_B[2]) * lambda_C * 0.618
            if self.hl > 2 :
                self.loss_style_B = self.criterionIdt(self.style_fake_B[1], self.style_B[1]) * lambda_C * 0.318
        elif self.opt.which_model_netG == 'unet_fuse_att_cca':
            if self.hl > 0 :
                self.loss_style_B = self.criterionIdt(self.style_fake_B[5], self.style_B[5]) * lambda_C * 0.918
            if self.hl > 1 :
                self.loss_style_B += self.criterionIdt(self.style_fake_B[4], self.style_B[4]) * lambda_C * 0.618
            if self.hl > 2 :
                self.loss_style_B += self.criterionIdt(self.style_fake_B[3], self.style_B[3]) * lambda_C * 0.318
        else:
            if self.hl > 0 :
                self.loss_style_B = self.criterionIdt(self.style_fake_B[5], self.style_B[5]) * lambda_C * 0.918
            if self.hl > 1 :
